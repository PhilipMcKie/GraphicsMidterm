...
public class Lab7 extends JPanel implements GLEventListener {
	...
	private void triangularPrism(GL2 gl2) {
		...
		gl2.glTexCoord2d(1.0, 0);
		gl2.glVertex3d(-t,-0.5,-0.25);
		gl2.glTexCoord2d(1.0, 1);
		gl2.glVertex3d(-t,0.5,-0.25);
		...
		gl2.glTexCoord2d(0, 1.0);
		gl2.glVertex3d(-t,0.5,-0.25);
		gl2.glTexCoord2d(0, 0);
		gl2.glVertex3d(-t,-0.5,-0.25);
		gl2.glTexCoord2d(1.0/3.0, 0);
		gl2.glVertex3d(0,-0.5,0.5);
		gl2.glTexCoord2d(1.0/3.0, 1.0);
		gl2.glVertex3d(0,0.5,0.5);
		gl2.glEnd();
	...
	private Texture textureFromResource(String resourceName) {
		...
		BufferedImage bri = null;
		Texture txt = null;
		
		try {
			
			bri = ImageIO.read(new File(resourceName));
			ImageUtil.flipImageVertically(bri);
			txt = AWTTextureIO.newTexture(displayGL.getGLProfile(), bri, true);
			txt.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
			txt.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
	...
	private Texture textureFromPainting() {
		...
		GL2 gl2 = context.getGL().getGL2();
		Texture txt; 
		BufferedImage bi = paintPanel.copyOSC();
		ImageUtil.flipImageVertically(bi);
		txt = AWTTextureIO.newTexture(displayGL.getGLProfile(), bi, true);
		txt.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
		txt.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
	...
	public void display(GLAutoDrawable drawable) {    
		...
		if(currentTexture == null) {
			...
			gl2.glLoadIdentity();
			gl2.glScaled(1.0 * textureRepeatHorizontal, 1.0*textureRepeatVertical, 1);
